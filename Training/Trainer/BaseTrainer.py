...
from Utils.visualize_utils import SaveAndVisualize
from Utils.visualize_pam import VisualizePAM, save_heatmaps
...
class BaseTrainer:
    ...
    It also plays an important role in XtrAIn optimization, which relies on run-
    ning only on a single model: the method ensures the model's state is always 
    up-to-date for the next training batch to run. That is because, the XtrAIn 
    efficient calculation requires returning back to the previous model state 
    
    BaseTrainer builds the following pipeline:
    1. First, the model is trained and the updated state is stored. XtrAIn --> model back to original state
    2. Then, the target loss is performed and the model is updated.  XtrAIn --> model back to original state
    3. Then the non-target loss is performed and the model is updated.  XtrAIn --> model back to original state
    4. After these steps, the model gets its updated state from step 1 to continue training.
    5. All scores are stored.
    ...
    def init_rand_step(self):
        ...
        for capturing their attribution scores. Also initializes the visualizer.
        """
        
        # Perform first, artificial step.
        rand_samples = RandomSamples(self.model, self.test_load, 
                                     self.epochs, 
                                     self.classes,
                                     self.break_per_epochs,
                                     self.n_test, self.data_pth,
                                     self.save_r_scores)
        
        self.X, self.labels = rand_samples.X, rand_samples.lbls
        ...
        self.R_norm = rand_samples.artificial_step()
        ...
        if self.dataset == 'pam50':
            self.save_viz = VisualizePAM(self.X.detach().cpu(), self.labels, self.epochs,
                                         self.classes, self.run_on + f"/run_{self.run_num}")

        else:
            self.save_viz = SaveAndVisualize(self.X.detach().cpu(), self.labels, self.epochs, 
                                         self.break_per_epochs, self.classes,
                                         self.run_on + f"/run_{self.run_num}", 
                                         self.save_pdf)
    ...
    def save_vizualization(self, epoch):
        ...
        if self.save_r_scores != 'none':
            ...
            if self.dataset == 'pam50':
                ...
                num_samp = 20
                save_heatmaps(self.save_viz.r_scores, self.save_viz.r_target_pos, 
                              [range(num_samp), range(num_samp)], self.save_viz.tot_pth, 
                              False, num_samp)
